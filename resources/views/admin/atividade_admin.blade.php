            ...
            <div class="table-responsive-xl">
                ...
                @endphp
                    <a class="fill-div" data-toggle="modal" data-target="#exampleModal{{$atividade->id}}"><div id="my-div" class="bd-callout @if($dataAtiv==$data && $atividade->data_entrega>$dataHora) bd-callout-warning @else @if($atividade->data_entrega>$dataHora) bd-callout-info @else @if($atividade->data_entrega<$dataHora) bd-callout-danger @endif @endif @endif">
                        <h4>{{$atividade->turma->serie}}º ANO {{$atividade->turma->turma}} - {{$atividade->disciplina->nome}} - {{$atividade->descricao}} - Entrega: {{date("d/m/Y H:i", strtotime($atividade->data_entrega))}}</h4>
...
<br>
<a href="/pedagogico" class="btn btn-success"data-toggle="tooltip" data-placement="bottom" title="Voltar"><i class="material-icons white">reply</i></a>
@endsection
